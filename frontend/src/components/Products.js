            ...
            axios.get(`https://www.googleapis.com/books/v1/volumes?q=flower&filter=paid-ebooks&key=${API_KEY}&maxResults=40`)
                .then(response => {
                    ...
                    booksUpdated.map(book => {
                        return (
                            axios.post('https://react-final-project-backend.vercel.app/api/products/', {
        ...
        const getProducts = async () => {
            try {
                const res = await axios.get(
                    cat
                        ? `https://react-final-project-backend.vercel.app/api/products?category=${cat}`
                        : "https://react-final-project-backend.vercel.app/api/products"
                );
